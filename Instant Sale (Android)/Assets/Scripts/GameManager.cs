...
public class GameManager : MonoBehaviour {
    public Text moneyText, netWorthText, debtText, rawIncomeText;
    ...
    public Text creditLimitText, loanAmountText, loanTotalText, defaultsText, transactionsText;
    public Text cardsLeftText;
    public Text[] tenancyTexts;

    public int money, rent, score;
    public int cardsLeft, cardsLeftMax;
    public int year, month, week;
    public int weeksPerMinute, maxWeeksPerMinute = 0;
    public int minCost = 3, maxCost = 15, rentMax = 75, totalDebt, networth, addMoneyAmnt, creditLimit;
    ...
    public Sprite warning, normal;

    public bool music = false, sounds = false;
    ...
    public Button musicToggle, soundToggle;
    ...
    public void CheckPile() {
        if (CardPile.childCount > 0) { //Are there any cards currently in the pile?
    ...
    public void AddMoney(int amount, string purchaseType) {
        ...
        if(amount != 0){
            ...
            transactionsText.text = purchaseType + ": ($" +amount.ToString("#,##0") + ") | $" + money.ToString("#,##0") + "\n" + oldText;
        }
    ...
    public void NextWeek() {
        ...
        yearsText.text = "W" + week.ToString() + " : M" + month.ToString() + " : Y" + year.ToString();
        
        TakeRepayments();
        CheckTenantTerms(); //Checks term amount and rent payments
    ...
    public void GenerateManagersForHire(){
        int x = Random.Range(6, 13);

        for (int i = 0; i < x; i++){
            GameObject mObj = Instantiate(managerObject, managersForHirePanel.transform);

            mObj.transform.GetChild(1).GetComponent<Toggle>().group = managersForHirePanel.GetComponent<ToggleGroup>();
            mObj.name = "manager_" + i.ToString();

            Manager m = mObj.GetComponent<Manager>();

            m.gameManager = this;

            m.weeklyPay =  Random.Range(100,250);
            m.hireCost = Random.Range(1000,5000);
            m.bonusAmount = (float)System.Math.Round(Random.Range(1.01f, 1.08f), 2);
            m.gameManager = this;
            m.description += "- $" + m.hireCost + " to hire, $" + m.weeklyPay.ToString("#,##0") + " weekly pay\n\n- Auto find tenants\n- Increases rent by " + ((m.bonusAmount - 1) * 100).ToString("F2") + "%"; //Change to randomly generate description
            mObj.transform.GetChild(1).GetComponent<Image>().sprite = managerSprites[Random.Range(0, 40)];

            managersForHire.Add(mObj);
        }
    }
    ...
    public void SelectManager(GameObject manager){
        ...
        if(m.hired){
            //Manager is an emplyee, fill out the selectedHiredManagerPanel
            selectedHiredManagerPanel.transform.GetChild(0).GetComponentInChildren<Image>().sprite = manager.transform.transform.GetChild(1).GetComponent<Image>().sprite;
            ...
            selectedManagerPanel.transform.GetChild(0).GetComponentInChildren<Image>().sprite = manager.transform.transform.GetChild(1).GetComponent<Image>().sprite;
    ...
    public void HireManager(){
        ...
        Manager m = selectedManager.GetComponent<Manager>();

        if(money >= m.hireCost){
            AddMoney(-m.hireCost, "Staff Hire Payment");
            ...
            selectedManager.transform.GetChild(1).GetComponent<Toggle>().group = currentManagersPanel.GetComponent<ToggleGroup>();

            GameObject duplicateManager = Instantiate(selectedManager, transform);
            duplicateManager.transform.SetParent(propertyManagersPanel.transform);
            duplicateManager.name = selectedManager.name;
            duplicateManager.transform.GetChild(1).GetComponent<Toggle>().group = propertyManagersPanel.GetComponent<ToggleGroup>();
    ...
    public void FireManager(GameObject manager){
        ...
        foreach (GameObject panel in buyPanels){
            if(panel.transform.childCount>1){
                ...
                if(c.tenantTermRemaining == 0){
                    ...
                    c.transform.parent.GetComponent<BuyPanel>().openPropertySlotButton.GetComponent<Image>().sprite = warning;
                ...
                if(week == c.tenantMoveInWeek){
                    ...
                    tenancyTexts[x].text = c.tenantTermRemaining.ToString();
    ...
    public void TakeRepayments(){
        if(Loans.Count > 0){
            ...
            foreach(KeyValuePair<int, int[]> loan in Loans){
                ...
                if(defaults < 3){ //Check the loan hasn't defaulted
                    int paymentWeek = loan.Value[5];

                    if(week == paymentWeek && loan.Value[2] > loan.Value[3]){ //Check if this week is the payment week
                        int repayments = loan.Value[7];

                        if(repayments > money && amountOwing > money){ //Check if the user has defaulted on this loan
                            loan.Value[4]++; //Add a strike to the defaults and move to the next loan
                            continue;
                        }

                        //Check if this is out final payment
                        if(repayments >= amountOwing){ //This payment will put us in credit{
    ...
    public void UpdateLoanText(int key){
        ...
        loansAccountText[key].text = "Payable: $" + (Loans[key][2] - Loans[key][3]).ToString("#,##0") + "\nMonthly: $" + Loans[key][7].ToString("#,##0") + " (" + Loans[key][5].ToString() + ")\nTerm: " + Loans[key][0].ToString();
    ...
    public void IncreaseWeeksPerSecond(){
        if(weeksPerMinute < maxWeeksPerMinute){
                ...
                InvokeRepeating("RepeatingWeeks", (60 - weeksPerMinute)*timeBuffer, (60 - weeksPerMinute)*timeBuffer);
        }
    }
    ...
    public void TakeLoan(){
        ...
        if(creditLimit >= amnt /*&& (rent * 4) < repayments*/){ //Maybe in the future, add rental income as a factor?
    ...
    public void UpdateLoanAmountSliderText(){
        ...
        int amountPayable = CalculateAmountPayable(GetSelectedToggle(), (int)loanAmountSlider.value);
        loanTotalText.text = "Total Repayable: $" + amountPayable.ToString("#,##0") + "\nTotal Interest: $" + (amountPayable - loanAmountSlider.value).ToString("#,##0") + "\n Monthly Repayments: $" + (amountPayable / GetSelectedToggle()).ToString() + " p/m";
    }
    ...
    public void CalculateScore(){
        int score = GameStats["TotalPropertiesOwned"] + (GameStats["TotalMoneySpent"] / 100000) + GameStats["TotalPropertiesSold"] + (GameStats["MoneySpentOnUpgrades"] / 100000) + GameStats["TotalPassedProperties"] + GameStats["TotalUpgrades"] + (GameStats["MostExpensiveProperty"] / 100000) + (GameStats["MostExpensivePurchased"] / 100000) +(GameStats["HighestRental"] / 10000) +GameStats["TotalNumberTenants"] + GameStats["TotalMonthsStayed"] + (GameStats["HighestNetworth"] / 100000) + (GameStats["MostAmountOfMoney"] / 100000);

        finalScoreText.text = "Score: " + score.ToString("#,##0");

        primaryStatsText.text = "Properties Owned: " + GameStats["TotalPropertiesOwned"] + "\n" + 
                                "Properties Sold:" + GameStats["TotalPropertiesSold"] + "\n" + 
                                "Properties Skipped: " + GameStats["TotalPassedProperties"] + "\n\n" +
                                "Total Spend On Upgrades: $" + GameStats["MoneySpentOnUpgrades"].ToString("#,##0") + "\n" +
                                "Total Upgrades Purchased: " + GameStats["TotalUpgrades"] + "\n" + 
                                "Total Money Spent: $" + GameStats["TotalMoneySpent"].ToString("#,##0");

        otherStatsText.text= "Most Expensive Property Owned: $" + GameStats["MostExpensiveProperty"].ToString("#,##0") + "\n" + 
                             "Most Expensive Property Purchased: $" + GameStats["MostExpensivePurchased"].ToString("#,##0") + "\n" +
                             "Highest Rental Cost: $" + GameStats["HighestRental"].ToString("#,##0") + "\n" +
                             "Number Of Tenants Housed: " + GameStats["TotalNumberTenants"] + "\n" +
                             "Combined Months Tenants Have Leased: " + GameStats["TotalMonthsStayed"] + "\n\n" +
                             "Highest Networth Obtained: $" + GameStats["HighestNetworth"].ToString("#,##0") + "\n" +
                             "Most Amount Of Money Had: $" + GameStats["MostAmountOfMoney"].ToString("#,##0");
